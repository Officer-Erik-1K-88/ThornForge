...
#
# Changes:
# Swapped `twine` mentions for `thornforge`.
# Replaced twine deps with thornforge deps.
import argparse
...
def dispatch(argv: List[str]) -> Any:
    registered_commands = importlib_metadata.entry_points(
        group="thornforge.registered_commands"
    )

    parser = argparse.ArgumentParser(prog="thornforge")
    parser.add_argument(
        "--version",
        action="version",
        version=f"%(prog)s version {thornforge.__version__} ({dep_versions()})",
    )
    parser.add_argument(
        "--no-color",
        default=False,
